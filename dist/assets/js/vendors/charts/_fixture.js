...
GetFixtures(localStorage.getItem("fixtureWeek"), localStorage.getItem("fixtureLeague"), localStorage.getItem("LeagueName"));
...
$.get("./assets/data/leagues.json", function (data) {
	...
	for (var i = 0; i < datas.length; i++) {
		html += '<div class="item" data-value="' + datas[i].logo + '">' + datas[i].league + "</div>";
	}
		...
		onChange: function onChange(value, $text) {
			...
			$.get("./assets/data/" + value + "/weeks.json", function (weekdata) {
				for (var i = 0; i < weekdata.RECORDS.length; i++) {
					weekhtml += '<div class="item" data-value="' + weekdata.RECORDS[i].week + '">Week ' + weekdata.RECORDS[i].week + "</div>";
				}
				$(".week-dd .menu").html(weekhtml);
				$(".week-dd").dropdown({
					onChange: function onChange(value1) {
						localStorage.setItem("fixtureWeek", value1);
						GetFixtures(value1, localStorage.getItem("fixtureLeague"), localStorage.getItem("LeagueName"));
					}
				});
			});
			GetFixtures(localStorage.getItem("fixtureWeek"), value, localStorage.getItem("LeagueName"));
		}
	});
});
...
function GetFixtures(weeko, leaguePath, leagueName) {
	$.get("./assets/data/" + leaguePath + "/fixture.json", function (data) {
		...
		var group = fixtureData[0].map(function (item) {
			return item.week_id;
			...
			var new_list = fixtureData[0].filter(function (itm) {
				return itm.week_id == item;
			}).map(function (itm) {
				return itm;
			});
			return {
		...
		for (var i = 0; i < weeksData[0].datas.length; i++) {
			...
			barData1Responsive.push({
				label: {
					...
					rich: {
						logo: {
							backgroundColor: {
								image: "./assets/img/leagues/" + leaguePath + "/team-logo/" + weekInfo.t1logo + ".png"
							},
							width: 50,
							height: 50
						},
			...
			barData2.push({
				...
				label: {
					...
					rich: {
						logo: {
							backgroundColor: {
								image: "./assets/img/leagues/" + leaguePath + "/team-logo/" + weekInfo.t2logo + ".png"
							},
							width: 50,
							height: 50
						},
			...
			barData3.push({
				...
				label: {
					...
					fontSize: 14,
					color: "#fdc203",
					formatter: weekInfo.hscore + "-" + weekInfo.ascore + "\n" + moment(weekInfo.date, "DD/MM/YYYY HH:mm").format("DD.MM.YYYY") + "\n" + moment(weekInfo.date, "DD/MM/YYYY HH:mm").format("HH:mm")
				}
			});
		}
		SeriesData.push({
	...
	fixtureChart.on("click", "series.bar", function (params) {
		if (params.componentType === "series") {
			if (params.seriesType === "bar") {
				window.location = "/single-game.html?gameId=" + encodeURIComponent(params.seriesName) + "";
			}
		}
	});
}