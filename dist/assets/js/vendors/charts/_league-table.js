...
$(".table-segment").css("background", "rgba(" + localStorage.getItem("chartColor") + ",1)");
var SaveIcon = "./assets/img/template-images/screenshot.svg";
//League Select Function
$(".ui.dropdown.league-select").dropdown({
  onChange: function onChange(value, text) {
    localStorage.setItem("league", value);
    GetTable(value, localStorage.getItem("week"), localStorage.getItem("chartColor"));
...
$(".ui.dropdown.week-select").dropdown({
  onChange: function onChange(value, text) {
    ...
    GetTable(localStorage.getItem("league"), value, localStorage.getItem("chartColor"));
...
$(".ui.dropdown.color-select").dropdown({
  onChange: function onChange(value) {
    localStorage.setItem("chartColor", value);

    $(".table-segment").css("background", "rgba(" + value + ",1)");

    GetTable(localStorage.getItem("league"), localStorage.getItem("week"), value);
  }
}).dropdown("set selected", localStorage.getItem("chartColor"));

GetTable(localStorage.getItem("league"), localStorage.getItem("week"), localStorage.getItem("chartColor"));

function GetTable(league, weeko) {
  var chartColor = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "rgba(55, 0, 60,.6)";
  ...
  $.get("./assets/data/" + league + "/table.json", function (data) {
    ...
    $.map(data.weeks, function (n, i, index) {
      weekMenu += '<div class="item" data-value="' + i + '"><img src="./assets/img/template-images/calendar.svg" class="ui avatar image calendar-image"/>Week ' + i.replace("week", "") + "</div>";
      $(".ui.dropdown.week-select").dropdown("set selected", localStorage.getItem("week"));
    });
    ...
    for (var i = 0; i < sorted.length; i++) {
      barItemStyle.push({
        ...
        label: {
          ...
          rich: {
            start1: {
              backgroundColor: {
                image: "./assets/img/leagues/" + league + "/team-logo/" + sorted[i].logo + ".png"
              },
          ...
          formatter: function formatter(value) {
            return "  {black|" + (value.dataIndex + 1) + ".}    {start1|}    {black|" + value.name + "}";
          }
        }
      });
      barData.push({
        ...
        tooltip: {
          ...
          position: "right",
          padding: 10,
          extraCssText: "margin-left:-40px;background:linear-gradient(180deg, " + sorted[i].color2 + " 50%, " + sorted[i].color1 + " 50%)",
          formatter: function formatter(value) {
            var type = "";
      ...
      bar4ItemStyle.push({
        ...
        tooltip: {
          ...
          extraCssText: "margin-left:-80px;margin-top:20px",
          formatter: function formatter(value) {
            return "Next Round<hr/>" + value.data.name.name + "<br />" + value.data.name.date + "";
          }
        },
        label: {
          color: "black",
          ...
          rich: {
            win: {
              backgroundColor: {
                image: "./assets/img/leagues/" + league + "/team-logo/" + sorted[i].next_play.logo + ".png"
              },
              width: 30,
              height: 30
            }
      ...
      datas.push([i, 5, sorted[i].win + sorted[i].draw + sorted[i].lost], [i, 6, sorted[i].win], [i, 7, sorted[i].draw], [i, 8, sorted[i].lost], [i, 9, sorted[i].gf], [i, 10, sorted[i].ga], [i, 11, sorted[i].gf - sorted[i].ga], [i, 12, sorted[i].win * 3 + sorted[i].draw]);
    }

    var hours = ["Pos.", "Club", "", "", "", "Play", "Won", "Draw", "Lost", "GF", "GA", "GD", "Pts", "Form", "", "Next"];
    ...
    option = {
      ...
      title: {
        text: localStorage.getItem("title1") + "-" + localStorage.getItem("title2"),
        left: "17",
        top: 10,
        textStyle: {
