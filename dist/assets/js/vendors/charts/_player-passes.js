...
GetPasses(localStorage.getItem("passesPlayer"), localStorage.getItem("passesColor"));
...
function GetPasses(pl1, clr) {
	$.get("./assets/data/player-passes.json", function (data) {
		...
		for (var i = 0; i < data.players.length; i++) {
			player1Names += '<div class="item" data-value="' + data.players[i].identy + '"><i class="icon circle"></i>' + data.players[i].name + "</div>";
		}
		...
		var player_image = "./assets/img/footballers/" + p1Data[0].identy + ".png";
		var option = {
			baseOption: {
				textStyle: {
				...
				radar: [{
					...
					name: {
						...
						formatter: function formatter(text, index) {
							if (text === "RIGHT") {
								return "{lblstyle|" + direction[0] + "}   {right|}";
							} else if (text === "BACK") {
								return "{lblstyle|" + direction[1] + "}\n\n{back|}";
							} else if (text === "LEFT") {
								return "{left|}   {lblstyle|" + direction[2] + "}";
							} else if (text === "FORWARD") {
								return "{forward|}\n\n{lblstyle|" + direction[3] + "}";
							}
						}
					},
					indicator: [{
