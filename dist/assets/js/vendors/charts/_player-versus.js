...
GetVersus(localStorage.getItem("player1"), localStorage.getItem("player2"), localStorage.getItem("pvColor"));
myChart.showLoading();
	...
	$.get("./assets/data/player-versus.json", function (data) {
		...
		for (var i = 0; i < data.players.length; i++) {
			if (data.players[i].identy != pl2) {
				player1Names += '<div class="item" data-value="' + data.players[i].identy + '"><i class="icon circle ' + clr + '"></i>' + data.players[i].name + "</div>";
			}
		}
		...
		for (var i = 0; i < data.players.length; i++) {
			if (data.players[i].identy != pl1) {
				player2Names += '<div class="item" data-value="' + data.players[i].identy + '"><i class="icon circle"></i>' + data.players[i].name + "</div>";
			}
		}
		$(".pl2 .menu").html(player2Names);

		for (var i = 0; i < data.colors.length; i++) {
			colorItems += '<div class="item" data-value="' + data.colors[i].value + '"><i class="icon circle ' + data.colors[i].class + '"></i>' + data.colors[i].name + "</div>";
		}
		...
		var option = {
			baseOption: {
				...
				radar: {
					...
					name: {
						show: true,
						formatter: function formatter(indicator, value) {
							if (value.text == "Overall") {
								return "{a|" + p1Data[0].data[0] + "}{c|/}{b|" + p2Data[0].data[0] + "\n}{x|Overall}";
							} else if (value.text == "Dribbling") {
								return "{a|" + p1Data[0].data[1] + "}{c|/}{b|" + p2Data[0].data[1] + "\n}{x|Dribbling}";
							} else if (value.text == "Finishing") {
								return "{a|" + p1Data[0].data[2] + "}{c|/}{b|" + p2Data[0].data[3] + "\n}{x|Finishing}";
							} else if (value.text == "Shot Power") {
								return "{a|" + p1Data[0].data[3] + "}{c|/}{b|" + p2Data[0].data[3] + "\n}{x|Shot Power}";
							} else if (value.text == "Stamina") {
								return "{a|" + p1Data[0].data[4] + "}{c|/}{b|" + p2Data[0].data[4] + "\n}{x|Stamina}";
							} else if (value.text == "Positioning") {
								return "{a|" + p1Data[0].data[5] + "}{c|/}{b|" + p2Data[0].data[5] + "\n}{x|Positioning}";
							}
						},
						rich: {
							x: {
					...
					splitLine: {
						lineStyle: {
							width: 2,
							color: ["rgba(224,134,82, 0.1)", "rgba(224,134,82, 0.2)", "rgba(224,134,82, 0.4)", "rgba(224,134,82, 0.6)", "rgba(224,134,82, 0.8)", "rgba(224,134,82, 1)"].reverse()
						}
					},
...
$(".pl1").dropdown({
	onChange: function onChange(value) {
		...
		GetVersus(value, localStorage.getItem("player2"), localStorage.getItem("pvColor"));
	}
...
$(".pl2").dropdown({
	onChange: function onChange(value) {
		...
		GetVersus(localStorage.getItem("player1"), value, localStorage.getItem("pvColor"));
	}
}).dropdown("set selected", localStorage.getItem("player2"));

$(".ui.dropdown.color").dropdown({
	onChange: function onChange(value) {
		localStorage.setItem("pvColor", value);
		GetVersus(localStorage.getItem("player1"), localStorage.getItem("player2"), value);
	}
