...
$(".ui.dropdown.team1").dropdown({
	onChange: function onChange(value, text) {
		localStorage.setItem("ts1", value);
		GetPlayers(value, localStorage.getItem("ts2"), localStorage.getItem("color"));
	}
}).dropdown("set selected", localStorage.getItem("ts1"));
$(".ui.dropdown.team2").dropdown({
	onChange: function onChange(value) {
		localStorage.setItem("ts2", value);
		GetPlayers(localStorage.getItem("ts1"), value, localStorage.getItem("color"));
	}
...
$(".ui.dropdown.color").dropdown({
	onChange: function onChange(value) {
		...
		GetPlayers(localStorage.getItem("ts1"), localStorage.getItem("ts2"), value);
	}
}).dropdown("set selected", localStorage.getItem("color"));

GetPlayers(localStorage.getItem("ts1"), localStorage.getItem("ts2"), localStorage.getItem("color"));
	...
	$.get("./assets/data/teams-compare.json", function (response) {
		...
		for (var i = 0; i < newDatas[0].length; i++) {
			if (newDatas[0][i].identy != pl2) {
				playerItems1 += '<div class="item" data-value="' + newDatas[0][i].identy + '">' + newDatas[0][i].name + "</div>";
			}
		}
		for (var i = 0; i < newDatas[0].length; i++) {
			if (newDatas[0][i].identy != pl1) {
				playerItems2 += '<div class="item" data-value="' + newDatas[0][i].identy + '">' + newDatas[0][i].name + "</div>";
			}
		}
		for (var i = 0; i < response.colors.length; i++) {
			colorItems += '<div class="item" data-value="' + response.colors[i].value + '"><i class="icon circle ' + response.colors[i].class + '"></i>' + response.colors[i].name + "</div>";
		}
